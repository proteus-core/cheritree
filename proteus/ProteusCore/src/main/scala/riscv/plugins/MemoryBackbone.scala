...
class MemoryBackbone(implicit config: Config) extends Plugin with MemoryService {
  ...
  private var internalDBusCtrl: MemBusControl = null
  ...
  override def createInternalDBus(stage: Stage): MemBusControl = {
    if (internalDBus == null) {
      stage plug new Area {
        internalDBus = master(new MemBus(config.dbusConfig)).setName("dbus")
        internalDBusCtrl = new MemBusControl(internalDBus).setName("dbusCtrl")
      }

      internalDBusStage = stage
    } else {
      assert(internalDBusStage == stage)
    }
