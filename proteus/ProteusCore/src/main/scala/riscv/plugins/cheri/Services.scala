...
trait TaggedMemoryService {
  ...
  def createCapScanIo(stage: Stage): CapScanIo
}

trait RegisterFileReadIo {
  def read(cs: UInt): Capability
}

trait RegisterFileService {
  /**
    * Add IO port to read register values.
    *
    * This method must be called during `setup`.
    *
    * Be aware that using this IO port might interfere with the reading of
    * registers in the decode stage. Therefore, it is only safe to do this while
    * ensuring that no instructions are being decoded. This could be achieved
    * by using [[ScheduleService.claimPipeline()]].
    */
  def addExternalReadIo(stage: Stage): RegisterFileReadIo
}
